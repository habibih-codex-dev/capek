  ...
  group(gid) {
    if (!this.data.groups[gid]) {
      this.data.groups[gid] = {
        ...
        welcomeText: "",
        goodbyeText: "",
        antilink: false, // hapus semua link (silent)
        antilinkv2: false, // hapus + kick (silent)
        antilinkwa: false, // hapus link grup WA (silent)
        antilinkch: false, // hapus link channel WA (silent)
        warnlink: false, // share link -> warn (ada notif), >limit -> kick
        antitoxic: false,
        antibot: false,
        antiforeign: false,
        onlyadmin: false,
        ...
        warnLimit: 3,
        kota: "", // kota jadwal sholat (kosong = pakai default config)
        autosholat: false, // pengingat sholat otomatis
        autosholatSound: false, // kirim audio adzan saat waktunya
      };
      this.markDirty();
    }
    // Jaga kompatibilitas data lama: pastikan field baru selalu ada
    const g = this.data.groups[gid];
    if (g.warns === undefined) g.warns = {};
    if (g.warnLimit === undefined) g.warnLimit = 3;
    if (g.kota === undefined) g.kota = "";
    for (const k of [
      "antilinkv2",
      "antilinkwa",
      "antilinkch",
      ...
      if (g[k] === undefined) g[k] = k.endsWith("Text") ? "" : false;
    }
    return g;
  }
